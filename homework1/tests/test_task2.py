...
def test_integer_mult(float1, float2, expected):
    assert task2.float_division(float1, float2) == expected

# Tests: Strings
@pytest.mark.parametrize("str, expected", [("friends", "sdneirf"), ("five hundred twenty", "ytnewt derdnuh evif"), (" ", " ")])
def test_integer_mult(str, expected):
    assert task2.string_invert(str) == expected

# Test: Booleans
@pytest.mark.parametrize("str, expected", [("HASTINGS", True), ("hastings", False), ("", False)])
def test_integer_mult(str, expected):
    assert task2.is_capitalized(str) == expected
